...
export default class HomeCompetitionList extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  callCompetitionListApi = async () => {
    ...
    try {
      // console.log("homeCompetitionListResponse to server ######################: ", JSON.stringify(formData));
      // console.log('!@#!@$!@$!@#'+Links.COMPETITION_LIST)
      const res = await fetch(Links.COMPETITION_LIST, {
        method: 'POST',
        body: formData,
        headers: {
      ...
      Toast.show(strings.somethingWentWrong, Toast.SHORT);
      console.log("36 Exception in API call77777: " + error);
    }
  ...
  setItemClick = (item) => {
    ...
    this.props.navigation.navigate('HomeCompetitionAwardList', { competitionCode: item.idx});
  ...
  render() {
    return (
      <SafeAreaView style={styles.container}>
        <Header title={this.state.pageTitle} navigation={this.props.navigation} />
